export const DISLIKE_FONT = "UNLIKE_FONT";
export const FETCH_DISPLAY_FONTS = "FETCH_DISPLAY_FONTS";
export const HIDE_PAGE_UP_BUTTON = "HIDE_PAGE_UP_BUTTON";
export const LIKE_FONT = "LIKE_FONT";
export const LOAD_ALL_FONTS = "LOAD_ALL_FONTS";
export const LOAD_FAVORITE_FONTS = "LOAD_FAVORITE_FONTS";
export const RECEIVE_FONTS = "RECEIVE_FONTS";
export const REQUEST_FONTS = "REQUEST_FONTS";
export const RESET_FONT_OPTIONS = "RESET_FONT_OPTIONS";
export const SHOW_PAGE_UP_BUTTON = "SHOW_PAGE_UP_BUTTON";
export const UPDATE_FAVES = "UPDATE_FAVES";
export const UPDATE_FONT_SIZE = "UPDATE_FONT_SIZE";
export const UPDATE_SAMPLE_TEXT = "UPDATE_SAMPLE_TEXT";
export const UPDATE_SEARCH_TERM = "UPDATE_SEARCH_TERM";
export const USER_LOGIN_SUCCESS = "USER_LOGIN_SUCCESS";
export const USER_LOGOUT = "USER_LOGOUT";
